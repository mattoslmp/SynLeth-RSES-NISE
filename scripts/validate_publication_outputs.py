...
def main() -> None:
  ...
  parser.add_argument("--run-marker", default=None, help="Optional file written immediately before the publication run; registered outputs must be at least as new.")
  args = parser.parse_args()
  article_root = resolve_path(args.article_root)
  run_marker = resolve_path(args.run_marker) if args.run_marker else None
  marker_mtime = run_marker.stat().st_mtime if run_marker and run_marker.exists() else None
  errors: list[str] = []
  ...
  require(manifest_path, errors); require(table_manifest_path, errors)
  if errors:
    raise SystemExit("\n".join(errors))
  ...
  if expected_supplementary - observed:
    errors.append(f"missing_supplementary_figures:{sorted(expected_supplementary - observed)}")
  if "layout_status" not in figures or not figures["layout_status"].eq("pass").all():
    failed = figures.loc[~figures.get("layout_status", pd.Series("missing", index=figures.index)).eq("pass"), [column for column in ("figure_id", "layout_status", "layout_warnings") if column in figures]]
    errors.append("layout_audit_failed:" + failed.to_json(orient="records"))

  for record in figures.to_dict("records"):
    base = Path(str(record["base_path"])); base = base if base.is_absolute() else ROOT / base
    for extension in ("png", "pdf", "svg"):
      rendered = base.with_suffix(f".{extension}")
      require(rendered, errors, minimum_size=100); freshness_targets.append(rendered)
    audit_path = base.with_suffix(".layout_audit.json")
    require(audit_path, errors); freshness_targets.append(audit_path)
  ...
  workbook = article_root / "workbooks/RSES_Onco_Article_Tables_and_Evidence.xlsx"
  require(workbook, errors, minimum_size=1000); freshness_targets.append(workbook)
  ...
  if marker_mtime is not None:
    for path in sorted(set(freshness_targets)):
      if path.exists() and path.stat().st_mtime + 1.0 < marker_mtime:
        errors.append(f"stale_output:{path}:mtime={path.stat().st_mtime}:marker={marker_mtime}")
