...
analysis() {
  ...
  run_logged "$LOG_DIR/02_recompute_full_extended_rses.log" \
  ...
  if [[ -s "$DEPMAP_RANKING" ]]; then
    ...
    run_logged "$LOG_DIR/03_recompute_depmap_extended_rses.log" \
      python -u scripts/recompute_rses_with_extended_multiomics.py \
  ...
  run_logged "$LOG_DIR/04_validate_extended_multiomics_analysis.log" \
...
publication() {
  ...
  run_logged "$LOG_DIR/05_make_extended_multiomics_figures.log" \
  ...
  run_logged "$LOG_DIR/08_validate_extended_multiomics_publication.log" \
    python -u scripts/validate_extended_multiomics_integrity.py \
      --processed-dir "$EXTENDED_PROCESSED_DIR" \
      --ranking "$FULL_RANKING" \
      --article-root "$ARTICLE_ROOT" \
