...
def main() -> None:
  ...
  for required in ("ranking", "candidates"):
    if not paths[required].exists() or paths[required].stat().st_size == 0:
      raise FileNotFoundError(f"Missing or empty mandatory table input: {paths[required]}")
  ...
  records.append(write_table(significant_dependency(dependency, args.fdr), main_dir / configured_main[2], "main", [paths["dependency"]]))
  top_pharmacology = pharmacology_ranking.sort_values("therapeutic_hypothesis_score", ascending=False).head(args.top_n * 3) if not pharmacology_ranking.empty and "therapeutic_hypothesis_score" in pharmacology_ranking else pharmacology_ranking
  ...
  external_sources = {
    "Table_S19_coverage_domain_source_cancer_class.tsv": ("scripts/build_publication_evidence_audit.py", [output_root / "tables/qc/coverage_by_domain_cancer_class.tsv"]),
    "Table_S20_score_decomposition_and_coverage.tsv": ("scripts/build_publication_evidence_audit.py", [output_root / "tables/score_components/rses_onco_score_decomposition.tsv"]),
    "Table_S21_evidence_overlap_and_roles.tsv": ("scripts/build_publication_evidence_audit.py", [output_root / "tables/qc/evidence_overlap_registry.tsv"]),
    "Table_S22_candidate_domain_missingness_audit.tsv": ("scripts/build_publication_evidence_audit.py", [output_root / "tables/qc/candidate_domain_evidence_audit.tsv"]),
  ...
  expected = len(configured_main) + len(configured_supplementary)
  if len(records) != expected:
    raise RuntimeError(f"Expected {expected} article tables; observed {len(records)}")
