#!/usr/bin/env bash
# Verify the complete real-data, evidence, Circos, structural and publication workflow.
set -Eeuo pipefail
...
log_stage "Validate the complete Circos-aware publication package"
MPLBACKEND=Agg \
...
supplementary_figures = int(
  figures["figure_id"].astype(str).str.match(
    r"^Figure_S(?:[1-9]|[1-6][0-9]|70)$"
  ).sum()
)
image_files = sum(
  1
...
if provenance_path.exists():
  ...
  print(
    f"git_status_porcelain: {provenance.get('git_status_porcelain')!r}"
  )
PY
