...
SUPPRESSIVE_TERMS = {
  "inhibitor", "inhibition", "antagonist", "blocker", "degrader", "negative modulator",
  "suppressor", "antisense", "sirna", "rnai",
}
...
def suppression_compatibility(rows: pd.DataFrame) -> float | None:
  texts = []
  for column in ("interaction_type", "action_type", "mechanism_of_action", "directionality"):
    if column in rows:
...
def cancer_relevance(rows: pd.DataFrame, cancer: str) -> float | None:
  ...
  disease_text = " ; ".join(
    rows.get("disease_name", pd.Series(dtype=object)).dropna().astype(str).str.casefold()
  )
  if not disease_text:
    civic = rows.loc[rows.get("source", pd.Series(index=rows.index, dtype=object)).astype(str).eq("civic")]
    if not civic.empty and civic.get("civic_gene_record", pd.Series(dtype=object)).astype(str).str.casefold().isin({"1", "true", "yes"}).any():
      return 0.45
    return None
...
def collect_drug_groups(evidence: pd.DataFrame) -> dict[str, list[dict[str, Any]]]:
  ...
  for record in evidence.to_dict("records"):
    ...
    identifier = record.get("drug_id")
    key = drug_key(name) or drug_key(identifier)
    if not key:
      key = "TARGET_ONLY"
    record = dict(record)
    record["drug_key"] = key
...
def merge_mychem(evidence: pd.DataFrame) -> pd.DataFrame:
  ...
  if mychem.empty or primary.empty:
    return evidence
  mychem["drug_key"] = mychem.get("drug_id", pd.Series(index=mychem.index, dtype=object)).map(drug_key)
  annotations = {
    key: record.get("raw_record")
    for key, record in mychem.dropna(subset=["drug_key"]).drop_duplicates("drug_key").set_index("drug_key").to_dict("index").items()
  }
  primary["drug_key"] = primary.apply(
    lambda row: drug_key(row.get("drug_name")) or drug_key(row.get("drug_id")), axis=1
  )
  primary["mychem_annotation"] = primary["drug_key"].map(annotations)
  return pd.concat([primary, mychem], ignore_index=True)
...
def candidate_drug_rows(
  ...
  if not sensitivity.empty:
    ...
    for key, group in sensitivity.groupby(["pair_id", "cancer", "drug_key"], dropna=False):
      sensitivity_groups[(str(key[0]), str(key[1]), str(key[2]))] = group
  ...
  for candidate in ranking.to_dict("records"):
    ...
    target_evidence = drug_groups.get(target, [])
    ...
    for key, records in by_drug.items():
      ...
      if not sensitivity_rows.empty:
        sensitivity_evidence = sensitivity_rows.copy()
        sensitivity_evidence["target_gene"] = target
        rows = pd.concat([rows, sensitivity_evidence], ignore_index=True, sort=False)
      ...
      if compatibility is not None:
        ...
        components["direct_interaction"] = (
          compatibility if direct is None else float(np.mean([direct, compatibility]))
        )
      pharmacology = coverage_aware_pharmacology_score(components)
      vulnerability = candidate.get("coverage_adjusted_rses")
      combined = therapeutic_hypothesis_score(vulnerability, pharmacology.adjusted_score)
      drug_name_values = rows.get("drug_name", pd.Series(dtype=object)).dropna().astype(str)
      drug_id_values = rows.get("drug_id", pd.Series(dtype=object)).dropna().astype(str)
      drug_name = drug_name_values.mode().iloc[0] if not drug_name_values.empty else None
      drug_id = drug_id_values.mode().iloc[0] if not drug_id_values.empty else None
      sources = ";".join(sorted(set(rows.get("source", pd.Series(dtype=object)).dropna().astype(str))))
      max_phase_values = [normalize_phase(value) for value in rows.get("max_phase", pd.Series(dtype=object))]
      max_phase_values = [value for value in max_phase_values if value is not None]
      ...
      output.update({
        ...
        **{f"pharmacology_component_{domain}": value for domain, value in components.items()},
        ...
        "maximum_clinical_phase_normalized": max(max_phase_values) if max_phase_values else None,
        "repurposing_candidate": bool(
          drug_name
          and max(max_phase_values, default=0.0) >= 0.5
          and (compatibility is None or compatibility >= 0.5)
        ),
        "research_only": True,
        "interpretation_boundary": (
          "Computational experimental-priority hypothesis; not evidence of clinical efficacy, "
          "patient benefit or cure. Requires biomarker-matched validation."
        ),
      })
...
def main() -> None:
  ...
  result = result.sort_values(
    ["therapeutic_hypothesis_score", "coverage_adjusted_rses", "pharmacology_adjusted"],
    ascending=[False, False, False],
    na_position="last",
  )
  result.to_csv(output_dir / "pharmacology_ranked_hypotheses.tsv", sep="\t", index=False)

  actionability_columns = [
    column for column in [
      "analysis_target_gene", "target_gene", "drug_name", "drug_id",
  ...
  result[[
    column for column in [
      ...
      "drug_name", "drug_id", "coverage_adjusted_rses",
      "pharmacology_adjusted", "therapeutic_hypothesis_score",
      *component_columns,
    ] if column in result.columns
  ]].to_csv(
  ...
  summary = {
    ...
    "unique_targets": result.get("target_gene", pd.Series(dtype=object)).nunique(),
    "unique_drugs": result.get("drug_name", pd.Series(dtype=object)).nunique(),
