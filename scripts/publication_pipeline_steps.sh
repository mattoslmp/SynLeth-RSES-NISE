#!/usr/bin/env bash
# Implementation of the RSES-Onco pharmacology, evidence-audit, robustness, structure and publication workflow.
set -Eeuo pipefail
...
preflight_common() {
  log_stage "Validate mandatory analysis inputs and code syntax"
  ...
  bash -n scripts/publication_pipeline_steps.sh
  printf '%s\n' "$(date -Iseconds)" > "$RUN_MARKER"
  echo "Publication run marker: $RUN_MARKER"
}

preflight_assets_only() {
  preflight_common
  log_stage "Validate cached evidence and structural inputs required by assets-only"
...
require_ranking() { require_file "$RANKING"; }
...
acquire_pharmacology() {
  ...
  run_logged "$LOG_DIR/01_acquire_pharmacology.log" \
    python -u scripts/acquire_pharmacology_evidence.py \
      --ranking "$RANKING" --output-dir "$PHARMACOLOGY_DATA" \
...
standardize_sensitivity() {
  ...
  run_logged "$LOG_DIR/02_standardize_drug_sensitivity.log" \
    python -u scripts/standardize_drug_sensitivity.py \
      --config config/drug_sensitivity_sources.yaml --models "$MODELS" \
...
analyze_sensitivity() {
  ...
  run_logged "$LOG_DIR/03_analyze_drug_response_selectivity.log" \
    python -u scripts/analyze_drug_response_selectivity.py \
      --gene-effect "$GENE_EFFECT" --copy-number "$COPY_NUMBER" --models "$MODELS" \
      --ranking "$RANKING" --pharmacology-evidence "$PHARMACOLOGY_DATA/pharmacology_evidence_long.tsv" \
      --sensitivity "$PHARMACOLOGY_DATA/drug_sensitivity_long.tsv" \
      --loss-threshold "$LOSS_THRESHOLD" --min-group-size "$MIN_GROUP_SIZE" \
...
prioritize_pharmacology() {
  ...
  run_logged "$LOG_DIR/04_prioritize_pharmacology.log" \
    python -u scripts/prioritize_pharmacology.py \
      --ranking "$RANKING" --evidence "$PHARMACOLOGY_DATA/pharmacology_evidence_long.tsv" \
      --sensitivity "$PHARMACOLOGY_DATA/drug_response_selectivity.tsv" --output-dir "$PHARMACOLOGY_RESULTS"
}
...
build_evidence_audit() {
  ...
  run_logged "$LOG_DIR/04b_build_publication_evidence_audit.log" \
    python -u scripts/build_publication_evidence_audit_complete.py \
      ...
      --source-metadata-root data/raw --output-root "$ARTICLE_ROOT"
}
...
run_robustness() {
  ...
  run_logged "$LOG_DIR/04c_run_rses_robustness.log" \
    python -u scripts/run_rses_robustness_analyses.py \
      --ranking "$RANKING" --output-root "$ARTICLE_ROOT" --top-k 20 --weight-delta 0.20
}
...
export_supporting_evidence() {
  ...
  run_logged "$LOG_DIR/04d_export_supporting_evidence.log" \
    python -u scripts/export_supporting_evidence_tables.py \
      --ranking "$RANKING" --functional-evidence "$FUNCTIONAL_EVIDENCE" --output-root "$ARTICLE_ROOT"
}

materialize_extended_tables() {
  log_stage "Materialize extended supplementary evidence and reproduction tables"
  run_logged "$LOG_DIR/04i_materialize_extended_supplementary_tables.log" \
    python -u scripts/materialize_extended_supplementary_tables.py --article-root "$ARTICLE_ROOT"
  run_logged "$LOG_DIR/04j_build_asset_reproduction_registry.log" \
    python -u scripts/build_asset_reproduction_registry.py \
      --config config/article_assets.yaml --article-root "$ARTICLE_ROOT"
}
...
export_tables() {
  ...
  run_logged "$LOG_DIR/05_export_article_tables.log" \
    python -u scripts/export_article_tables.py \
      ...
      --structure-manifest "$STRUCTURE_MANIFEST" --structural-annotations "$STRUCTURAL_ANNOTATIONS" \
      --structure-render-manifest "$STRUCTURE_RENDER_MANIFEST" --output-root "$ARTICLE_ROOT"
}
...
generate_figures() {
  ...
  run_logged "$LOG_DIR/06_make_all_article_figures.log" \
    python -u scripts/make_all_article_figures.py \
      ...
      --render-manifest "$STRUCTURE_RENDER_MANIFEST" --structural-annotations "$STRUCTURAL_ANNOTATIONS" \
      --structural-coverage "$STRUCTURAL_COVERAGE" --output-root "$ARTICLE_ROOT" "$layout_flag"
}
...
build_workbook() {
  ...
  run_logged "$LOG_DIR/07_build_article_workbook.log" \
    python -u scripts/build_article_workbook.py \
      --article-root "$ARTICLE_ROOT" --output "$ARTICLE_ROOT/workbooks/RSES_Onco_Article_Tables_and_Evidence.xlsx"
}
...
build_manifests() {
  ...
  run_logged "$LOG_DIR/08_build_publication_manifest.log" \
    python -u scripts/build_publication_manifest.py \
      --article-root "$ARTICLE_ROOT" --input "$RANKING" --input "$CANDIDATES" \
      --input "$DISCOVERY" --input "$FUNCTIONAL_EVIDENCE" \
...
validate_outputs() {
  ...
  run_logged "$LOG_DIR/09_validate_publication_outputs.log" \
    python -u scripts/validate_publication_outputs.py \
      --article-root "$ARTICLE_ROOT" --run-marker "$RUN_MARKER"
  run_logged "$LOG_DIR/10_pytest.log" python -m pytest -q -p no:cacheprovider
}

build_documents() {
  log_stage "Build and render the manuscript and supplementary documents"
  run_logged "$LOG_DIR/11_build_publication_documents.log" \
    python -u scripts/build_publication_documents.py \
      --article-root "$ARTICLE_ROOT" --output-dir "$ARTICLE_ROOT/documents" \
      --render-pdf --render-pages
  build_manifests
  run_logged "$LOG_DIR/12_validate_publication_documents.log" \
    python -u scripts/validate_publication_documents.py \
      --article-root "$ARTICLE_ROOT" --document-dir "$ARTICLE_ROOT/documents" \
      --require-page-renders
}

all() {
  preflight_common; acquire_pharmacology; standardize_sensitivity; analyze_sensitivity
  prioritize_pharmacology; build_evidence_audit; run_robustness; export_supporting_evidence
  materialize_extended_tables; prepare_structures; export_tables; generate_figures
  catalog_figure_data; validate_scientific_integrity; build_workbook; build_manifests; validate_outputs
}

assets_only() {
  preflight_assets_only; prioritize_pharmacology; build_evidence_audit; run_robustness
  export_supporting_evidence; materialize_extended_tables; export_tables; generate_figures
  catalog_figure_data; validate_scientific_integrity; build_workbook; build_manifests; validate_outputs
}
...
Stages:
  ...
  extended-tables         Materialize extended supplementary and reproduction tables
  ...
  documents               Build, render and validate manuscript and supplement DOCX/PDF files
...
case "$stage" in
  ...
  extended-tables) materialize_extended_tables ;;
