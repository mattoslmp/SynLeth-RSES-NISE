...
A signed WGCNA network is built with biweight midcorrelation (`bicor`) as the primary correlation, `maxPOutliers=0.10`, and `pearsonFallback="individual"`. Pearson is therefore used only for an individual gene or module eigengene whose median absolute deviation is zero or non-finite; the remaining columns retain bicor. The same policy is applied to soft-threshold selection, signed adjacency and signed module membership (`kME`). Every fallback-eligible gene and eigengene, its MAD, and the per-cancer fallback counts are exported for audit. The soft-thresholding power is the first power with signed scale-free topology R2 at least 0.80 and negative slope; otherwise the finite power with the highest available R2 is selected. Signed adjacency, signed topological overlap, average-linkage clustering, dynamic tree cutting, module merging at eigengene dissimilarity 0.25, module membership and intramodular connectivity are calculated.
